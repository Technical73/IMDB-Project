...
export const AddActorModal = ({ actorOpen, handleClose }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await dispatch(createActor(formData)).unwrap();
      toast.success(response.message || "Actor added successfully!");
      handleClose();
    } catch (error) {
      setFormErrors({ submit: error || "Failed to add actor" });
      toast.error(error || "Failed to add actor");
    }
  };

  return (
    <Dialog open={actorOpen} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogTitle
        sx={{
          display: "flex",
